...
def train_and_test(filepath="static/uploaded/logReg.txt", epsilon=0.1, split_ratio=1, epochs=100, **unused_args):
...
def make_and_save_graph(filepath="static/uploaded/logReg.txt", split_ratio=1, n_clusters=8, epochs=100, **unused_args):
	print("making pandas dataframe")
	df=pd.read_csv(filepath, header=None)
    print("head:",df.head())
    X=df.iloc[:,:-1].values #features, np array
